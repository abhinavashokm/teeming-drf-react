...
class OutcomeMetric(WorkspaceScopedBaseModel):
    ...
    unit = models.CharField(
        max_length=20, choices=UnitChoices, default=UnitChoices.NUMBER
    )
    direction = models.CharField(
        max_length=20, choices=DirectionChoices, default=DirectionChoices.INCREASE
    )
...
class OutcomeCheckIn(WorkspaceScopedBaseModel):
    ...
    status = models.CharField(
        max_length=20, choices=StatusChoices, default=StatusChoices.MEASURING
    )
    notes = models.TextField(blank=True, null=True)

    contributed_ideas = models.ManyToManyField(
        "idea.Idea",
        related_name="outcome_checkins",
        blank=True,
    )
