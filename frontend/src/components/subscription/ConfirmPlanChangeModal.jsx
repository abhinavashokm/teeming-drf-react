...
import useDowngradePlan from "../../hooks/subscription/useDowngradePlan";
...
export default function ConfirmPlanChangeModal({
    ...
    const { mutate: downgradePlan, isPending: isDowngradePending } = useDowngradePlan();

    const isPending = isUpgrade ? isUpgradePending : isDowngradePending;
    ...
    const handleConfirm = () => {
        ...
        mutate({ planId: plan.id }, { onSettled: onClose });
    };

    const formattedAmountDue =
        preview?.amountDue != null
            ? `${currencySymbols[preview.currency] || ""}${(preview.amountDue / 100).toFixed(2)}`
            : null;
            ...
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4">
                ...
                <div className="px-6 pt-6 pb-4">
                    <div className="flex items-center gap-3 mb-1">
                        <div className={`flex items-center justify-center w-9 h-9 rounded-full ${isUpgrade ? "bg-green-50" : "bg-orange-50"}`}>
                            {isUpgrade ? (
                    ...
                    <p className="text-[13px] text-gray-500 ml-12">
                        {isUpgrade
                            ? "Your card on file will be charged immediately for the remaining time in your current billing period."
                            : `Your plan will change to ${plan?.name} at the end of your current billing period. You'll be billed the new amount automatically from the next cycle.`
                        }
                    </p>
                </div>
                ...
                <div className="px-6 pb-4">
                    {isUpgrade ? (
                            ...
                            {!isPreviewLoading && previewError && (
                                <div className="rounded-xl border border-red-200 bg-red-50 p-3">
                                    <p className="text-[13px] text-red-600">
                                        Couldn't calculate the upgrade amount. Please try again.
                                    </p>
                                </div>
                            )}
                            ...
                            {!isPreviewLoading && !previewError && preview && (
                                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 flex flex-col gap-3">
                                    <div className="flex items-center justify-between">
                                        <span className="text-[13px] text-gray-500">Due today (prorated)</span>
                                        <span className="text-[15px] font-semibold text-gray-900">{formattedAmountDue}</span>
                                    </div>
                                    <div className="h-px bg-gray-200" />
                                    <div className="flex items-center justify-between">
                                        <span className="text-[13px] text-gray-500">Then billed monthly at</span>
                                        <span className="text-[13px] font-medium text-gray-700">
                                            {currencySymbols[plan?.currency]}{plan?.monthlyPrice}/month
                                        </span>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 flex flex-col gap-3">
                            <div className="flex items-center justify-between">
                                <span className="text-[13px] text-gray-500">New monthly price</span>
                            ...
                            <div className="flex items-center justify-between">
                                <span className="text-[13px] text-gray-500">Takes effect</span>
                                <span className="text-[13px] font-medium text-gray-700">At next billing cycle</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3 rounded-b-[16px]">
                    ...
                    <AppButton
                        ...
                        disabled={(isUpgrade && (isPreviewLoading || !!previewError)) || isPending}
                        loading={isPending}
                    >
                        {isUpgrade
                            ? formattedAmountDue
